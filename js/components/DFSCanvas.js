...
import { TILE_TYPES, TILE_COLORS, generateTileMap, findGoalDFS, mapHeight, mapWidth } from "/components/dfsLogic";
...
const DFSCanvas = () => {
  ...
  const drawPaths = async () => {
    ...
    const { searched, goalPath } = findGoalDFS(playerCoords, goalCoords, tileMap);

    if (!goalPath) {
      alert("no path found for dfs");
  ...
  return (
    <div>
      <canvas ref={canvasRef} id="dfsCanvas" width={500} height={500} />
