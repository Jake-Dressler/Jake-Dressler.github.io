...
export function findGoalH(startCoords, endCoords, tilemap) {
    ...
    function euclideanDistance(coords) {
        const dx = coords[0] - endCoords[0];
        const dy = coords[1] - endCoords[1];
        return Math.sqrt(dx * dx + dy * dy);
    }

    // Mark the start node as visited and enqueue it
    visited[startCoords[0]][startCoords[1]] = true;
    priorityQueue.push({ coords: startCoords, priority: euclideanDistance(startCoords) });

    // Iterate over the queue
    while (priorityQueue.length) {
        // Dequeue a cell from queue
        priorityQueue.sort((a, b) => a.priority - b.priority);
        const { coords: curr } = priorityQueue.shift();
        ...
        for (let n of neighbors) {
            ...
            if (!visited[ny][nx] && isValid(n, tilemap)) {
                visited[ny][nx] = true;
                priorityQueue.push({ coords: n, priority: euclideanDistance(n) });
